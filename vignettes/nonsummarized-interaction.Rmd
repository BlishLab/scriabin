...
```{r}
test <- RunNICHES(ifnb, species = "human")
```


```{r}
demo <- test$CellToCell
demo <- ScaleData(demo)
...
DimPlot(demo,reduction = 'umap',group.by = 'VectorType',label = F) + NoLegend()
```


```{r}
imputed <- SeuratWrappers::RunALRA(ifnb)
...
DimPlot(demo.2,reduction = 'umap',group.by = 'VectorType',label = T) + NoLegend()
