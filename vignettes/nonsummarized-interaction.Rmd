...
subseu <- subset(ifnb, cells = sample(colnames(ifnb),100))
InteractionModules <- function(object, assay = "SCT", slot = "data",
  ...
  if(ncol(object)>iterate.threshold) {
    ...
    mat_list <- pblapply(seq_along(1:n.rep), function(z) {
      ...
      y <- b[,colnames(x)]
      m <- sqrt(as.sparse((pbsapply(1:nrow(x), function(i) tcrossprod(x[i, ], y[i, ])))))
  ...
  dynamicMods = cutreeDynamic(dendro = geneTree,  
                              method="tree", minClusterSize = min.size);
  ...
  if(return.mat) {
    return(list(cor_mat = m_cor, modules = modules))
  }
  else {
    return(modules)
  }
...
# test <- corSparse(m)
# colnames(test) <- colnames(m)
# rownames(test) <- colnames(m)
# pdf("~/Downloads/p.pdf", height = 200, width = 200)
